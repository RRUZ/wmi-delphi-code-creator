...
{$ENDIF}
  type TWMiQualifierMetaData = class private FName: string;
FValue:
string;
public
  ...
  TWMiPropertyMetaData = class private FName: string;
  FDescription: string;
  FType: string;
  FCimType: Integer;
  FPascalType: string;
...
public
  ...
  TWMiMethodMetaData = class private FValidValues: TStrings;
  FValidMapValues: TStrings;
  FIsStatic: Boolean;
  FIsFunction: Boolean;
...
public
  ...
  TWMiClassMetaData = class private
{$IFDEF USEXML}
    FXmlDoc: OleVariant;
  FXml: string;
{$ENDIF}
...
{$ENDIF}
  ...
  overload;
  function GetProperty(const Name: string): TWMiPropertyMetaData;
  overload;
  function GetMethod(index: Integer): TWMiMethodMetaData;
  overload;
  function GetMethod(const Name: string): TWMiMethodMetaData;
  overload;
  ...
  function GetSystemProperty(index: Integer): TWMiPropertyMetaData;
  overload;
  function GetSystemProperty(const Name: string): TWMiPropertyMetaData;
  overload;
public
{$IFDEF USEXML}
...
{$ENDIF}
  constructor Create(const ANameSpace, AClass: string);
  overload;
  constructor Create(const ANameSpace, AClass, Host, User, Password: string);
  overload;
  Destructor Destroy;
  override;
  ...
  cdecl;
  overload;
  procedure GetListWMINameSpaces(const RootNameSpace: String; const List: TStrings; ReportException: Boolean = True);
  cdecl;
  overload;
  procedure GetListWMINameSpaces(const RootNameSpace, Host, User, Password: String; const List: TStrings;
  ...
  overload;
  procedure GetListWmiClasses(const NameSpace: String; const List: TStrings;
    const IncludeQualifiers, ExcludeQualifiers: Array of string; ExcludeEvents: Boolean);
  cdecl;
  overload;
  procedure GetListWmiClasses(const NameSpace, Host, User, Password: String; const List: TStrings;
    const IncludeQualifiers, ExcludeQualifiers: Array of string; ExcludeEvents: Boolean);
  cdecl;
  overload;
  ...
  procedure GetListWmiMethodInParameters(const NameSpace, WmiClass, WmiMethod: String;
    ParamsList, ParamsTypes, ParamsDescr: TStringList);
  procedure GetListWmiMethodOutParameters(const NameSpace, WmiClass, WmiMethod: String;
    ParamsList, ParamsTypes, ParamsDescr: TStringList);
...
begin
  ...
  if not VarIsNull(V) then
    case CimType of
      wbemCimtypeSint8, wbemCimtypeUint8, wbemCimtypeSint16, wbemCimtypeUint16, wbemCimtypeSint32, wbemCimtypeUint32,
        wbemCimtypeSint64, wbemCimtypeUint64, wbemCimtypeReal32, wbemCimtypeReal64, wbemCimtypeBoolean,
        wbemCimtypeString, wbemCimtypeChar16:
        Result := VarStrNull(V);
      wbemCimtypeDatetime:
        Result := DateTimeToStr(VarDateTimeNull(V));
...
begin
  case CimType of
    wbemCimtypeSint8:
      Result := wbemtypeSint8;
    wbemCimtypeUint8:
      Result := wbemtypeUint8;
    wbemCimtypeSint16:
      Result := wbemtypeSint16;
    wbemCimtypeUint16:
      Result := wbemtypeUint16;
    wbemCimtypeSint32:
      Result := wbemtypeSint32;
    wbemCimtypeUint32:
      Result := wbemtypeUint32;
    wbemCimtypeSint64:
      Result := wbemtypeSint64;
    wbemCimtypeUint64:
      Result := wbemtypeUint64;
    wbemCimtypeReal32:
      Result := wbemtypeReal32;
    wbemCimtypeReal64:
      Result := wbemtypeReal64;
    wbemCimtypeBoolean:
      Result := wbemtypeBoolean;
    wbemCimtypeString:
      Result := wbemtypeString;
    wbemCimtypeDatetime:
      Result := wbemtypeDatetime;
    wbemCimtypeReference:
      Result := wbemtypeReference;
    wbemCimtypeChar16:
      Result := wbemtypeChar16;
    wbemCimtypeObject:
      Result := wbemtypeObject;
  else
    Result := 'Unknow'
  end;
end;
...
function GetDefaultValueWmiType(const WmiType: string): string;
begin
  if WmiType = wbemtypeSint8 then
    Result := '0'
  else if WmiType = wbemtypeUint8 then
    Result := '0'
  else if WmiType = wbemtypeSint16 then
    Result := '0'
  else if WmiType = wbemtypeUint16 then
    Result := '0'
  else if WmiType = wbemtypeSint32 then
    Result := '0'
  else if WmiType = wbemtypeUint32 then
    Result := '0'
  else if WmiType = wbemtypeSint64 then
    Result := '0'
  else if WmiType = wbemtypeUint64 then
    Result := '0'
  else if WmiType = wbemtypeReal32 then
    Result := '0.0'
  else if WmiType = wbemtypeReal64 then
    Result := '0.0'
  else if WmiType = wbemtypeBoolean then
    Result := 'True'
  else if WmiType = wbemtypeString then
    Result := QuotedStr('')
  else if WmiType = wbemtypeDatetime then
    Result := 'Now'
  else if WmiType = wbemtypeReference then
    Result := 'varNull'
  else if WmiType = wbemtypeChar16 then
    Result := QuotedStr('')
  else if WmiType = wbemtypeObject then
    Result := 'varNull'
  else
    Result := ''
end;
...
begin
  if WmiType = wbemtypeSint8 then
    ...
    Result := 'OleVariant' // ???   IDispatch
  else if WmiType = wbemtypeChar16 then
    Result := 'WideString'
    /// ?????
  else if WmiType = wbemtypeObject then
    Result := 'OleVariant' // ????  variant
  else
    Result := 'Unknow'
end;
...
procedure GetListWMINameSpaces(const RootNameSpace, Host, User, Password: String; const List: TStrings;
  ReportException: Boolean = True); cdecl; overload;
var
  objSWbemLocator: OleVariant;
  objWMIService: OleVariant;
  colItems: OleVariant;
...
procedure GetListWMINameSpaces(const RootNameSpace: String; const List: TStrings; ReportException: Boolean = True);
  cdecl; overload; // recursive function
var
  objSWbemLocator: OleVariant;
  objWMIService: OleVariant;
  colItems: OleVariant;
...
begin
  ...
  for objClass in GetOleVariantEnum(colClasses) do
    ...
    begin
      if (CompareText(objClassQualifier.Name, 'dynamic') = 0) or
        (CompareText(objClassQualifier.Name, 'static') = 0) Then
      begin
...
procedure GetListWmiClasses(const NameSpace, Host, User, Password: String; const List: TStrings;
  const IncludeQualifiers, ExcludeQualifiers: Array of string; ExcludeEvents: Boolean); overload;
var
  objSWbemLocator: OleVariant;
  objWMIService: OleVariant;
  colClasses: OleVariant;
...
begin
  ...
  try
    ...
    begin
      ...
      if ExcludeEvents and (ExtrinsicWmiEvents.IndexOf(ClassName) > -1) then
        Continue;
      if ExcludeEvents and (IntrinsicWmiEvents.IndexOf(ClassName) > -1) then
        Continue;
...
constructor TWMiClassMetaData.Create(const ANameSpace, AClass, Host, User, Password: string);
begin
  inherited Create;
  FNameSpace := ANameSpace;
  FClass := AClass;
...
function TWMiClassMetaData.GetProperty(const Name: string): TWMiPropertyMetaData;
Var
  i, index: Integer;
begin
  Index := -1;
...
function TWMiClassMetaData.GetSystemProperty(const Name: string): TWMiPropertyMetaData;
Var
  i, index: Integer;
begin
  Index := -1;
...
function TWMiClassMetaData.GetSystemProperty(index: Integer): TWMiPropertyMetaData;
begin
  Result := FCollectionSystemPropertyMetaData[index];
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      else if SameText(VarStrNull(Qualif.Name), 'Singleton') then
        FIsSingleton := True
      else if SameText(VarStrNull(Qualif.Name), 'Association') then
        FIsAssociation := True
      else if SameText(VarStrNull(Qualif.Name), 'Description') then
        PropertyMetaData.FDescription := VarStrNull(Qualif.Value)
      else if SameText(VarStrNull(Qualif.Name), 'values') then
      begin
        if not VarIsNull(Qualif.Value) and VarIsArray(Qualif.Value) then
          for i := VarArrayLowBound(Qualif.Value, 1) to VarArrayHighBound(Qualif.Value, 1) do
            PropertyMetaData.FValidValues.Add(VarStrNull(Qualif.Value[i]));
      end
      else if SameText(VarStrNull(Qualif.Name), 'ValueMap') then
      begin
        if not VarIsNull(Qualif.Value) and VarIsArray(Qualif.Value) then
          for i := VarArrayLowBound(Qualif.Value, 1) to VarArrayHighBound(Qualif.Value, 1) do
            PropertyMetaData.FValidMapValues.Add(VarStrNull(Qualif.Value[i]));
  ...
  begin
    ...
    begin
      ...
      else if SameText(VarStrNull(Qualif.Name), 'Static') then
    ...
    for i := 0 to MethodMetaData.FInParameters.Count - 1 do
      if MethodMetaData.InParameters[i].IsArray then
        MethodMetaData.FMethodInParamsDecl := MethodMetaData.FMethodInParamsDecl +
          Format('%s : Array of %s -', [MethodMetaData.FInParameters[i].Name, MethodMetaData.InParameters[i].&Type])
      else
        MethodMetaData.FMethodInParamsDecl := MethodMetaData.FMethodInParamsDecl +
          Format('%s : %s -', [MethodMetaData.FInParameters[i].Name, MethodMetaData.InParameters[i].&Type]);
    ...
    for i := 0 to MethodMetaData.OutParameters.Count - 1 do
      if MethodMetaData.OutParameters[i].IsArray then
        MethodMetaData.FMethodOutParamsDecl := MethodMetaData.FMethodOutParamsDecl +
          Format('%s : Array of %s -', [MethodMetaData.OutParameters[i].Name, MethodMetaData.OutParameters[i].&Type])
      else
        MethodMetaData.FMethodOutParamsDecl := MethodMetaData.FMethodOutParamsDecl +
          Format('%s : %s -', [MethodMetaData.OutParameters[i].Name, MethodMetaData.OutParameters[i].&Type]);
    ...
    for i := 0 to MethodMetaData.InParameters.Count - 1 do
      if MethodMetaData.InParameters[i].IsArray then
        MethodMetaData.FMethodInParamsPascalDecl := MethodMetaData.FMethodInParamsPascalDecl +
          Format('const %s : Array of %s;', [EscapeDelphiReservedWord(MethodMetaData.InParameters[i].Name),
          WmiTypeToDelphiType(MethodMetaData.InParameters[i].&Type)])
      else
        MethodMetaData.FMethodInParamsPascalDecl := MethodMetaData.FMethodInParamsPascalDecl +
          Format('const %s : %s;', [EscapeDelphiReservedWord(MethodMetaData.InParameters[i].Name),
          WmiTypeToDelphiType(MethodMetaData.InParameters[i].&Type)]);
    ...
    for i := 0 to MethodMetaData.OutParameters.Count - 1 do
      if CompareText(MethodMetaData.OutParameters[i].Name, 'ReturnValue') <> 0 then
        MethodMetaData.FMethodOutParamsPascalDecl := MethodMetaData.FMethodOutParamsPascalDecl +
          Format('var %s : %s;', [EscapeDelphiReservedWord(MethodMetaData.OutParameters[i].Name),
          WmiTypeToDelphiType(MethodMetaData.OutParameters[i].&Type)]);
    ...
    if not MethodMetaData.FIsFunction then
    // delete default type value for method because is a method and not a function.
...
begin
  ...
  for i := 1 to lNodes do
  begin
    if CompareText(FXmlDoc.selectSingleNode(Format('/CLASS/PROPERTY[%d]/@CLASSORIGIN', [i])).Text, '___SYSTEM') <> 0
    then
    begin
      ...
      begin
        ...
        begin
          FPropertiesDescr[FPropertiesDescr.Count - 1] :=
            FXmlDoc.selectSingleNode(Format('/CLASS/PROPERTY[%d]/QUALIFIER[%d]/VALUE', [i, j])).Text;
          Break;
        end;
      end;
    end;
  ...
  begin
    ...
    begin
      ...
      if CompareText(Node.Text, 'Description') = 0 then
      begin
        FMethodsDescr[FMethodsDescr.Count - 1] :=
          FXmlDoc.selectSingleNode(Format('/CLASS/METHOD[%d]/QUALIFIER[%d]/VALUE', [i, j])).Text;
        Break;
      end;
    end;
    ...
    if not VarIsNull(NodesC) then
      ...
      begin
        ...
        begin
          ...
          else if CompareText(Node.Text, 'Out') = 0 then
          begin
            IdxOut := k;
            Break;
          end
        ...
        begin
          ...
          begin
            ...
            if CompareText(Node.Text, 'Description') = 0 then
            begin
              Node := FXmlDoc.selectSingleNode(Format('/CLASS/METHOD[%d]/PARAMETER[%d]/QUALIFIER[%d]/VALUE',
                [i, j, k]));
          ...
          begin
            ...
            for k := 1 to NodesQ.Length do
            begin
              Node := FXmlDoc.selectSingleNode(Format('/CLASS/METHOD[%d]/PARAMETER[%d]/QUALIFIER[%d]/@NAME',
                [i, j, k]));
              if CompareText(Node.Text, 'Description') = 0 then
              begin
                Node := FXmlDoc.selectSingleNode(Format('/CLASS/METHOD[%d]/PARAMETER[%d]/QUALIFIER[%d]/VALUE',
